  ...
  const handleLogin = async () => {
    try {
      ...
      if (response.request.status === 200) {
        const { token, user: loadedUser } = response.data;
        console.log("Toàn bộ response từ API:", response);
        console.log("Dữ liệu trong response.data:", response.data); 
        ...
        if (loadedUser.user_type=='customer') { 
          // Vai trò là spso
          navigate('/customerdashboard');
        } else if (loadedUser.user_type=='seller') {
          // Vai trò là student
